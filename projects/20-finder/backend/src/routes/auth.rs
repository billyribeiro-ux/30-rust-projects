//! Email + password auth routes. OAuth lives in routes::oauth.
...
async fn login(
    ...
    let (user_id, name, valid) = match row {
        Some(u) => {
            // OAuth-only accounts have NULL password_hash; treat as not
            // a valid email-login target. Still run a dummy verify to
            // keep timing constant.
            match u.password_hash.as_deref() {
                Some(h) => {
                    let ok = hash::verify_password(&input.password, h)?;
                    (u.id, u.name, ok)
                }
                None => {
