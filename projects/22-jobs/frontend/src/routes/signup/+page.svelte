...
<svelte:head><title>Create account — Jobs</title></svelte:head>
...
<main class="shell">
  <section class="card">
    ...
    >
      <label class="field">
        <span>Name</span>
        <input type="text" name="name" required autocomplete="name" value={(form?.name as string | undefined) ?? ''} />
      </label>
      <label class="field">
        <span>Email</span>
...
<style>
  ...
  .card { width: 100%; max-width: 420px; padding: var(--space-8) var(--space-6); background: var(--color-bg-elev); border: 1px solid var(--color-border); border-radius: var(--radius-lg); box-shadow: var(--shadow-md); display: grid; gap: var(--space-4); }
