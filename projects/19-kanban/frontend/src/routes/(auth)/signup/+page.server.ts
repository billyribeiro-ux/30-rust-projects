...
const BACKEND_URL = process.env.VITE_BACKEND_URL ?? 'http://localhost:3018';
  ...
  default: async ({ request, fetch, cookies }) => {
    ...
    if (!res.ok) {
      const body = (await res.json().catch(() => null)) as
        ...
        | null;
      const message = body?.error?.message ?? `Signup failed (${res.status}).`;
      return fail(res.status, { email, name, error: message });
    }
