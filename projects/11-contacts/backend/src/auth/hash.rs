...
//!
//! Parameters per OWASP 2025 recommendations for interactive logins:
//!     m_cost = 19_456 KiB (~19 MiB), t_cost = 2, p_cost = 1, output = 32 bytes
//!
//! The Argon2id variant resists both side-channel attacks (the `i` half) and
//! GPU/ASIC parallelism (the `d` half). The PHC-format string we store
//! captures salt + parameters, so a hash never needs an external schema —
//! verifying is `Argon2::default().verify_password(input, &parsed_hash)`.
