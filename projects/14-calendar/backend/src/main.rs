...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "calendar_backend=info,tower_http=info".into()),
        )
        .with_target(false)
        .compact()
        .init();
    ...
    let frontend_origin =
        std::env::var("FRONTEND_ORIGIN").unwrap_or_else(|_| "http://localhost:5186".to_string());
    let smtp_url =
        std::env::var("SMTP_URL").unwrap_or_else(|_| "smtp://localhost:1025".to_string());
    let smtp_from =
        std::env::var("SMTP_FROM").unwrap_or_else(|_| "noreply@calendar.local".to_string());
    let public_url =
        std::env::var("PUBLIC_URL").unwrap_or_else(|_| "http://localhost:5186".to_string());
    ...
    let app = Router::new()
        ...
        .nest("/api/calendars", routes::calendars::router())
