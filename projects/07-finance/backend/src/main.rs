...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "finance_backend=info,tower_http=info".into()),
        )
        .with_target(false)
        .compact()
        .init();
    ...
    let frontend_origin =
        std::env::var("FRONTEND_ORIGIN").unwrap_or_else(|_| "http://localhost:5179".to_string());
    ...
    let app = Router::new()
        ...
        .nest("/api/imports", routes::imports::router())
