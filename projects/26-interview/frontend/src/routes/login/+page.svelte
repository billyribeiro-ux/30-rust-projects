...
<svelte:head><title>Sign in — Marketplace</title></svelte:head>
...
<main class="shell">
  <section class="card">
    ...
    <form method="POST" use:enhance={() => { submitting = true; return async ({ update }) => { submitting = false; await update(); }; }}>
      ...
      <label class="field"><span>Password</span><input type="password" name="password" required autocomplete="current-password" minlength="12" /></label>
