...
async fn list(State(pool): State<SqlitePool>) -> AppResult<Json<Vec<Member>>> {
    let rows =
        sqlx::query!("SELECT id, name, color, created_at FROM members ORDER BY created_at ASC")
            .fetch_all(&pool)
            .await?;
...
async fn create(
    ...
    sqlx::query!(
        "INSERT INTO members (id, name, color, created_at) VALUES (?1, ?2, ?3, ?4)",
        id,
        name,
        color,
        now_str,
...
async fn delete(State(pool): State<SqlitePool>, Path(id): Path<String>) -> AppResult<StatusCode> {
    let result = sqlx::query!("DELETE FROM members WHERE id = ?1", id)
        .execute(&pool)
        .await
        .map_err(|err| {
            if let sqlx::Error::Database(dbe) = &err
                && dbe.message().contains("FOREIGN KEY constraint failed")
            {
                return AppError::Fields(vec![FieldError {
                    field: "id".into(),
                    message: "cannot delete: member has expenses. remove expenses first."
                        .into(),
                }]);
            }
            AppError::Database(err)
