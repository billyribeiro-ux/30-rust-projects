...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        // 2FA step-up second stage. Lives under /api/auth/ rather than
        // /api/2fa/ because conceptually it's part of the login flow — the
        // request that finally creates the session.
        .route("/2fa/verify", post(crate::routes::twofa::step_up_verify))
}
...
/// Login can produce TWO different shapes:
///   - `LoginResult::Direct` — no 2FA, full UserPublic + session cookie set
///   - `LoginResult::Pending2fa` — 2FA is enabled; we return a short-lived
///     intermediate token. The client makes a second request to
///     /api/auth/2fa/verify with `{ intermediate, code }`.
///
/// Untagged so the JSON wire format is just "either a user, or an object
/// with requires_2fa: true".
#[derive(Debug, Serialize)]
#[serde(untagged)]
enum LoginResult {
    Direct(UserPublic),
...
async fn login(
    ...
    let row = sqlx::query!(
        r#"
        SELECT id, email, name, password_hash, email_verified_at, totp_enabled_at
    ...
    let (user_id, name, email_verified, totp_on, valid) = match row {
        Some(u) => {
    ...
    // 2FA on? Issue intermediate, DO NOT issue session yet.
    if totp_on {
        let intermediate = tokens::issue(&s.pool, user_id, TokenKind::TotpIntermediate).await?;
        return Ok((
            StatusCode::OK,
            jar,
            Json(LoginResult::Pending2fa {
                requires_2fa: true,
                intermediate,
            }),
        ));
    ...
    Ok((
        StatusCode::OK,
        jar,
        Json(LoginResult::Direct(UserPublic {
            id: user_id,
            email,
            name,
            email_verified,
