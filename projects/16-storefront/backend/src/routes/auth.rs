//! Admin auth routes. Customer-facing flow doesn't use these — customers
//! identify by email only, no password, no session.
//!
//! This is the bare minimum to log into the admin panel: register/login/
//! logout/me. We deliberately don't ship a `/forgot` UI here (the pattern
//! is taught in project 14), but the underlying tokens module exists.
...
async fn login(
    ...
    let (user_id, name, email_verified, valid) = match row {
        Some(u) => {
            ...
            (u.id, u.name, u.email_verified_at.is_some(), ok)
        }
        None => {
...
mod tests {
    ...
    fn email_rejects_malformed() {
        assert!(matches!(normalize_email("nope"), Err(AppError::Fields(_))));
    }
}
